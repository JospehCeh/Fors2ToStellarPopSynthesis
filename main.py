...
from fors2tostellarpopsynthesis.fitters.fitter_jaxopt import (
    SSP_DATA, get_infos_comb, get_infos_mag, get_infos_spec, lik_mag,
    lik_normspec_from_mag, lik_spec, lik_spec_from_mag, lik_ugri, mean_mags,
    mean_sfr, mean_spectrum, mean_ugri, ssp_spectrum_fromparam)
from fors2tostellarpopsynthesis.fitters.fitter_util import (
    plot_fit_ssp_photometry, plot_fit_ssp_spectrophotometry,
    plot_fit_ssp_spectrophotometry_sl, plot_fit_ssp_spectroscopy,
    plot_fit_ssp_ugri, plot_SFH, rescale_photometry, rescale_spectroscopy,
    rescale_starlight_inrangefors2)
from fors2tostellarpopsynthesis.fors2starlightio import (
    Fors2DataAcess, SLDataAcess, convert_flux_torestframe, gpr)
# ## Parameters to fit
...
# ## Attempt with fewer parameters and age-dependant, fixed-bounds metallicity
    ...
    lbfgsb_ugri = jaxopt.ScipyBoundedMinimize(fun = lik_ugri, method = "L-BFGS-B", maxiter=5000)
    lbfgsb_mag = jaxopt.ScipyBoundedMinimize(fun = lik_mag, method = "L-BFGS-B", maxiter=5000)
    lbfgsb_spec = jaxopt.ScipyBoundedMinimize(fun = lik_normspec_from_mag, method = "L-BFGS-B", maxiter=5000)
    lbfgsb_comb = jaxopt.ScipyBoundedMinimize(fun = lik_comb, method = "L-BFGS-B", maxiter=5000)
    ...
    for tag in tqdm(dict_fors2_for_fit):
        ...
        = rescale_photometry(dict_params_m,\
        ...
        = rescale_spectroscopy(dict_params_m,\
        ...
        '''
        # plot SFR
        plot_SFH(dict_params_m, data_dict["redshift"], subtit = data_dict["title"], ax=None)
        ...
        '''
        xphot_rest, yphot_rest, eyphot_rest, factor\
        = rescale_photometry(dict_params_s,\
                                         data_dict["wl_mean_filters"],\
                                         data_dict["mags"],\
                                         data_dict["mags_err"],\
                                         data_dict["redshift"])
        ...
        #rescale Fors2 spectroscopy
        Xspec_data_rest, Yspec_data_rest, EYspec_data_rest, factor\
        = rescale_spectroscopy(dict_params_mm,\
                                           data_dict["wavelengths"],\
                                           data_dict["fnu"],\
                                           data_dict["fnu_err"],\
                                           data_dict["redshift"])
        ...
        axs = axs.ravel()
        plot_SFH(dict_params_mm, data_dict["redshift"], subtit = data_dict["title"], ax=axs[0])
        ...
        filename_params = f"DSPS_pickles/fitparams_{tag}_galex_successive_fits.pickle"
