...
DEFAULT_AGEDEPLGMET_PARAMS_MIN =  np.array([-2., -4.34])
DEFAULT_AGEDEPLGMET_PARAMS_MAX = np.array([3., 0.])
...
DEFAULT_SCALE_PARAMS_MAX = np.array([1.0e+12])

# bound parameters together
DEFAULT_PARAMS = [DEFAULT_MAH_PARAMS,\
                  DEFAULT_MS_PARAMS,\
                  DEFAULT_Q_PARAMS,\
                  DEFAULT_DUST_PARAMS,\
...
PARAMS_MIN = np.concatenate(([DEFAULT_MAH_PARAMS_MIN,\
...
PARAMS_MAX = np.concatenate(([DEFAULT_MAH_PARAMS_MAX,\
...
DICT_PARAM_AGEDEPLGMET_true = OrderedDict([(AGEDEPLGMET_PARAMNAMES[0], DEFAULT_AGEDEPLGMET_PARAMS[0]),\
...
class SSPParametersFit():
    ...
    def __init__(self):
        ...
        # bound parameters together
        self.DEFAULT_PARAMS = [self.DEFAULT_MAH_PARAMS,\
                               self.DEFAULT_MS_PARAMS,\
                               self.DEFAULT_Q_PARAMS,\
                               self.DEFAULT_DUST_PARAMS]#,\
        #                       self.DEFAULT_SCALE_PARAMS]

        self.PARAMS_MIN = np.concatenate(([self.DEFAULT_MAH_PARAMS_MIN,\
                                           self.DEFAULT_MS_PARAMS_MIN,\
                                           self.DEFAULT_Q_PARAMS_MIN,\
                                           self.DEFAULT_DUST_PARAMS_MIN]))#,\
        #                                   self.DEFAULT_SCALE_PARAMS_MIN]))
        self.PARAMS_MAX = np.concatenate(([self.DEFAULT_MAH_PARAMS_MAX,\
                                           self.DEFAULT_MS_PARAMS_MAX,\
                                           self.DEFAULT_Q_PARAMS_MAX,\
                                           self.DEFAULT_DUST_PARAMS_MAX]))#,\
        #                                   self.DEFAULT_SCALE_PARAMS_MAX]))
        ...
        self.PARAM_NAMES = [self.MAH_PARAMNAMES,\
                            self.MS_PARAMNAMES,\
                            self.Q_PARAMNAMES,\
                            self.DUST_PARAMNAMES]#, self.SCALE_PARAMNAMES]
        self.PARAM_NAMES_FLAT = list(itertools.chain(*self.PARAM_NAMES))
        
        self.DICT_PARAM_MAH_true = OrderedDict([(name, self.DEFAULT_MAH_PARAMS[k])\
                                                for k, name in enumerate(self.MAH_PARAMNAMES)\
                                               ])
        
        self.DICT_PARAM_MS_true = OrderedDict([(name, self.DEFAULT_MS_PARAMS[k])\
                                               for k, name in enumerate(self.MS_PARAMNAMES)\
                                              ])
        
        self.DICT_PARAM_Q_true = OrderedDict([(name, self.DEFAULT_Q_PARAMS[k])\
                                              for k, name in enumerate(self.Q_PARAMNAMES)\
                                             ])
        
        self.DICT_PARAM_DUST_true = OrderedDict([(name, self.DEFAULT_DUST_PARAMS[k])\
                                                 for k, name in enumerate(self.DUST_PARAMNAMES)\
                                                ])
        
        #self.DICT_PARAM_SCALE_true = OrderedDict([(name, self.DEFAULT_SCALE_PARAMS[k])\
        #                                         for k, name in enumerate(self.SCALE_PARAMNAMES)])
    ...
    def __repr__(self) -> str:
        all_str = []
        all_str.append(f"Class SSPParametersFit_AgeDepMet_Q : ")
